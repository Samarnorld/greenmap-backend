...
app.get('/treecover-per-ward', async (req, res) => {
  try {
    console.log('🌳 /treecover-per-ward called');

    const pixelArea = ee.Image.pixelArea();

    // Load wards FeatureCollection (already global as `wards` or load here)
    // const wards = ee.FeatureCollection("projects/greenmap-backend/assets/nairobi_wards_filtered");

    // Fetch the most recent date in Dynamic World data
    const dwCollection = ee.ImageCollection('GOOGLE/DYNAMICWORLD/V1').select('label');

    // Get max date of collection, then extract year
    const latestImage = await dwCollection
      .sort('system:time_start', false)
      .first()
      .getInfo();

    if (!latestImage || !latestImage.properties || !latestImage.properties['system:time_start']) {
      return res.status(500).json({ error: 'No Dynamic World data available' });
    }

    const latestDate = new Date(latestImage.properties['system:time_start']);
    const latestYear = latestDate.getUTCFullYear();

    // Use the latest available year or ?year param (if provided and valid)
    const requestedYear = parseInt(req.query.year);
    const year = (requestedYear && requestedYear <= latestYear) ? requestedYear : latestYear;

    console.log(`Using tree cover data for year: ${year}`);

    const start = ee.Date.fromYMD(year, 1, 1);
    const end = start.advance(1, 'year');

    // Filter Dynamic World to year and select tree class (1)
    const yearlyDw = dwCollection.filterDate(start, end);

    // Mode composite to get dominant class per pixel in that year
    const treeMask = yearlyDw.mode().eq(1).selfMask();

    // Tree area image in m²
    const treeAreaImage = treeMask.multiply(pixelArea).rename('tree_m2');

    // Reduce tree area per ward polygon
    const treeAreaPerWard = treeAreaImage.reduceRegions({
      collection: wards,
      reducer: ee.Reducer.sum(),
      scale: 10,
      tileScale: 4
    ...
    // Calculate tree cover % per ward
    const treeCoverPerWard = treeAreaPerWard.map(f => {
      const wardArea = f.geometry().area();
      const tree_m2 = ee.Number(f.get('sum')).max(0);
      const tree_pct = tree_m2.divide(wardArea).multiply(100);
      return f.set({
        'tree_m2': tree_m2,
        'ward_area_m2': wardArea,
        'tree_pct': tree_pct
    ...
    const geojson = await treeCoverPerWard.getInfo();

    res.setHeader('Cache-Control', 'public, max-age=3600'); // cache 1 hour
    res.json({
      year,
      updated: new Date().toISOString(),
      wards: geojson.features.map(f => ({
        ward: f.properties.NAME_3 || f.properties.ward || 'Unknown',
        tree_m2: f.properties.tree_m2,
        ward_area_m2: f.properties.ward_area_m2,
        tree_pct: f.properties.tree_pct
      }))
    });

  } catch (error) {
    console.error('❌ /treecover-per-ward error:', error);
    res.status(500).json({ error: 'Failed to compute tree cover per ward', details: error.message });
  }
});
