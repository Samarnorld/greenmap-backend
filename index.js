...
// Charttrend: city-wide multi-year environmental trends
app.get('/charttrend', async (req, res) => {
  ...
  try {
    ...
    // parse years (defaults to 2021..current)
    ...
    const years = [];
    const ndviVals = [];   // server-side EE objects (scalar expressions)
    const treeVals = [];
    const builtPctVals = [];
    const rainVals = [];
    ...
    for (let y = startYear; y <= endYear; y++) {
      ...
      // period definitions
      const yearStart = ee.Date.fromYMD(y, 1, 1);
      const yearEnd = ee.Date.fromYMD(y, 12, 31);
      // dry-season window (June - Sep) — less vegetation & more stable for built-up detection
      const dryStart = ee.Date.fromYMD(y, 6, 1);
      const dryEnd = ee.Date.fromYMD(y, 9, 30);

      /* -------------------------
         Sentinel-2 composite (preferred dry-season; fallback to full-year)
         ------------------------- */
      const s2Dry = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
        .filterBounds(nairobiGeom)
        .filterDate(dryStart, dryEnd)
        .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 40)); // relaxable

      const s2Full = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
        .filterBounds(nairobiGeom)
        .filterDate(yearStart, yearEnd)
        .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 60));

      // safe "zero band" (ensures composite has B8/B4/B11 in pathological cases)
      const zeroS2 = ee.Image.constant([0, 0, 0]).rename(['B8', 'B4', 'B11']).toFloat();

      // Prefer dry-season mean; if empty, fall back to full-year mean; if still empty, use zeroS2.
      const s2Composite = ee.Image(ee.Algorithms.If(
        s2Dry.size().gt(0),
        s2Dry.mean(),
        ee.Algorithms.If(s2Full.size().gt(0), s2Full.mean(), zeroS2)
      ));

      // NDVI from Sentinel composite (will be -1..+1). We'll clip later to 0..1 in payload.
      const ndviS2 = s2Composite.normalizedDifference(['B8', 'B4']).rename('NDVI');

      // NDBI from S2: (B11 - B8) / (B11 + B8) — add small epsilon to denominator
      const b11 = s2Composite.select('B11');
      const b8 = s2Composite.select('B8');
      const ndbiS2 = b11.subtract(b8).divide(b11.add(b8).add(1e-9)).rename('NDBI');

      // NDVI mean across city (use 10m scale)
      const ndviVal = ndviS2.reduceRegion({
        reducer: ee.Reducer.mean(),
        ...
        tileScale: 2,
        bestEffort: true
      }).get('NDVI'); // server-side value (may be null)

      ndviVals.push(ndviVal);
      log.info(`NDVI ${y} queued`);

      /* -------------------------
         Tree coverage: Dynamic World 'tree' probability band mean (0..1)
         ------------------------- */
      const dw = ee.ImageCollection('GOOGLE/DYNAMICWORLD/V1')
        .filterDate(yearStart, yearEnd)
        .filterBounds(nairobiGeom);

      // Use probability 'tree' band averaged over the year
      const treeProb = dw.select('tree').mean();
      const treeFrac = treeProb.reduceRegion({
        reducer: ee.Reducer.mean(),
        geometry: nairobiGeom,
        scale: 10,
        maxPixels: 1e13,
      ...
      /* -------------------------
         Built-up percent (S2 dry-season composite)
         - Use native 10m scale for area sum
         - Stricter thresholds: NDBI > 0.15 AND NDVI < 0.25
         - Multiply pixelArea and sum
         ------------------------- */
      // mask candidate built pixels
      const builtMask = ndbiS2.gt(0.15).and(ndviS2.lt(0.25)).selfMask(); // stricter thresholds reduce false positives

      // built area image in m^2
      const builtAreaImage = builtMask.multiply(pixelArea).rename('built_m2');

      const builtSum = builtAreaImage.reduceRegion({
        reducer: ee.Reducer.sum(),
        geometry: nairobiGeom,
        scale: 10,              // use Sentinel native resolution
        maxPixels: 1e13,
        tileScale: 2,
        bestEffort: true
      }).get('built_m2');

      // built percentage (0..100). If builtSum is null, this will propagate as null.
      const builtPct = ee.Number(builtSum).divide(totalAreaM2).multiply(100);
      builtPctVals.push(builtPct);
      log.info(`Built-up ${y} queued`);

      /* -------------------------
         Rainfall: CHIRPS annual sum, reduce by mean across geometry
         ------------------------- */
    ...
    // Pack everything into a dictionary for single evaluate() call
    ...
    // Helper: normalize arrays into numbers or nulls (client-side)
    ...
    // Build payload and do final client-side fixes:
    // - NDVI: clamp to 0..1 (so chart scales remain stable)
    // - Tree coverage: convert 0..1 to percent 0..100 (if present)
    const rawNdvi = normalizeArray(rawResult.ndvi ?? rawResult.NDVI, n).map(v => (v === null ? null : Math.max(0, Math.min(1, v))));
    const rawTree = normalizeArray(rawResult.tree_frac, n).map(v => (v === null ? null : Number((v * 100).toFixed(3))));
    const rawBuilt = normalizeArray(rawResult.built_pct, n); // already percent (0..100) on server
    const rawRain = normalizeArray(rawResult.rainfall ?? rawResult.precipitation ?? rawResult.rain, n);

    const payload = {
      years: rawResult.years ?? years,
      ndvi: rawNdvi,
      tree_coverage: rawTree,
      built_up: rawBuilt,
      rainfall: rawRain
    };
